        ...
        if self.color:
            if self.exposed:
                ...
                canvas.draw_text(str(self.number),
                                 (self.pos[0]+0.5*card_WIDTH-0.5*NUMBER_SIZE,
                                  self.pos[1]+0.5*card_HEIGHT+0.5*NUMBER_SIZE),
                                 NUMBER_SIZE, 'Black')
            else:
                canvas.draw_line((self.pos[0]+0.5*card_WIDTH, self.pos[1]),
                                 (self.pos[0]+0.5*card_WIDTH, self.pos[1]+card_HEIGHT), card_WIDTH, 'WHITE')
...
class Player:
    ...
    def sort(self):
        ...
        for card in self.cards:
            card.change_pos(
                (self.pos[0]+self.cards.index(card)*card_WIDTH, self.pos[1]))
        return self.cards
    ...
    def predict(self, player, index):
        ...
        guess_list = [rank for rank in unknown_list if not (
            rank in my_known_list)]

        guess_list = [i for i in list(guess_list) if i %
                      2 == chosen_card.get_color()]

        n = len(player.cards)
        m = n - index
        guess_list = [i for i in list(
            guess_list) if i >= index and i <= 23-m+1]
...
def new_game():
    ...
    if total_games > max_games:
        hint_message.change_information("Game limit is reached")
        print("Winning Rate of Heuristic: " +
              str(((max_games-loss_count)/max_games)*100) + "%")
        return
...
def judge1(guess, player):
    if not chosen_card:
        ...
        guess_message.change_information(
            'Guess: '+str(player.cards.index(chosen_card))+' to be '+str(guess))
...
def out_process(player):
    ...
    hidden_cards = [
        card for card in player.cards if not card in player.exposed_cards()]
    if hidden_cards == []:
        player.out = True
    ...
    for i in range(player_numbers):
        if players[i].out:
            if i == 0:
                hint_message.change_information('Heuristic Wins!')
                timer2.start()
                ...
                if players[i].get_order() != player_alive_numbers:
                    for t in range((i+1) % (player_alive_numbers+1), player_alive_numbers+1):
                        temp = (players[t % player_alive_numbers].get_order(
                        )-1) % player_alive_numbers
...
def process(player1, player2):
    ...
    inturn_message.change_pos(
        (player1.pos[0]-90, player1.pos[1]+card_HEIGHT*0.5))
    # draw cards
    ...
    if can_deal[1]:
        hidden_cards = [
            card for card in player2.cards if not card in player2.exposed_cards()]
        if hidden_cards:
            ...
            chosen_mark = (chosen_card.get_pos()[
                           0]+10, chosen_card.get_pos()[1]-10)
            guess = max_p[0]
